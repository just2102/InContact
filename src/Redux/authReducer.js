...
const setCurrentUser = (id, login, email) => ({
  type: SET_CURRENT_USER,
  currentUser: { id, login, email },
});
  ...
  return function (dispatch) {
    ...
    authAPI.whoAmI().then((data) => {
      if (data.resultCode === 0) {
        dispatch(
          setCurrentUser(data.data.id, data.data.login, data.data.email)
        );
...
export function requestLogin(email, password, rememberMe) {
  return function (dispatch) {
